  ...
  return values.slice(headerIndex + 1)
    ...
    .map(function(row) {
      ...
      const scores = rawScores.map(function(score) {
        return {
          value: score.value,
          isInitial: score.index < 2 && score.value !== ''
        };
      });
...
function saveFightResult(date, fightNumber, result) {
  ...
  const existingRow = rowNumber
    ...
    : [];
  const slots = updateOverallScores(date, fight, result, existingRow[11], existingRow[12]);
...
function updateOverallScores(date, fight, result, leftSlot, rightSlot) {
  ...
  return {
    leftSlot: updateOverallEntryScore(date, fight.left.entryName, leftScore, leftSlot),
    rightSlot: updateOverallEntryScore(date, fight.right.entryName, rightScore, rightSlot)
  };
}

function updateOverallEntryScore(date, entryName, score, preferredSlot) {
  if (!entryName) return '';
  ...
  const slot = pickOverallSlot(sheet, targetRow, scoreStart, preferredSlot);
...
function pickOverallSlot(sheet, rowNumber, scoreStart, preferredSlot) {
  const savedSlot = Number(preferredSlot);
  if (savedSlot >= 3 && savedSlot <= 7) return savedSlot;
