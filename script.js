...
function showResultOverlay(title, text, details, isLoading) {
  ...
  return `
    <article class="fight-card">
      ...
      <div class="action-grid">
        <button class="result-button win-left" type="button" data-result="LEFT_WIN" data-fight="${escapeHtml(fight.fightNumber)}">Left Win</button>
        <button class="result-button draw" type="button" data-result="DRAW" data-fight="${escapeHtml(fight.fightNumber)}">Draw</button>
        <button class="result-button win-right" type="button" data-result="RIGHT_WIN" data-fight="${escapeHtml(fight.fightNumber)}">Right Win</button>
        <button class="result-button cancel" type="button" data-result="CANCEL" data-fight="${escapeHtml(fight.fightNumber)}">Cancel</button>
      </div>
...
function saveResult(fightNumber, result) {
  ...
  setMessage('');
  showResultOverlay('Saving Fight #' + fightNumber, label, '', true);

  callApi({ action: 'saveResult', date: selectedDate, fightNumber, result })
    .then(function(data) {
      results = data.results || results;
      renderAll();
      showResultOverlay('Fight #' + fightNumber, label, createOverlayDetails(fight, result), false);
    })
    .catch(function(error) {
...
function createOverlayDetails(fight, result) {
  ...
  if (result === 'DRAW' || result === 'CANCEL') {
    return `
      <div class="overlay-match overlay-even-result">
        <div>
...
document.querySelectorAll('[data-open-date]').forEach(function(button) {
  button.addEventListener('click', function() {
    showMonitor(button.dataset.openDate);
  });
});
